...
void line_1(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
...
void line_2(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
          tiny::image &image, tiny::color const& color) {
...
void line_3(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
          tiny::image &image, tiny::color const& color) {
    bool steep = false;
    ...
    for (int32_t x = x0; x <= x1; x++) {
        ...
        int32_t y = y0 * (1.f - t) + y1 * t;
        if (steep) image.set(y, x, color); // if transposed, de-tranpose
        else image.set(x, y, color);
    }

}

void line_4(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
          tiny::image &image, tiny::color const& color) {
    bool steep = false;
    if (std::abs(x0 - x1) < std::abs(y0 - y1)) {
        std::swap(x0, y0);
